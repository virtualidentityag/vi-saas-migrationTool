...
public class AgencyTopicMigrationService {
  ...
  public void addAgencyTopicRelationIfNeeded(Integer consultingTypeId) {
    ...
    agencies.forEach(
        agencyId -> {
          ...
          String sql =
              "INSERT INTO agency_topic (id, agency_id, topic_id) VALUES (NEXT VALUE FOR sequence_agency_topic, ?, ?)";
          agencyServiceJdbcTemplate.update(sql, agencyId, topicId);
          log.info(
              "Created agency topic relation for agencyId: {} and topicId: {}", agencyId, topicId);
        });
  }
  ...
  private boolean agencyTopicsRelationExists(Integer agencyId, Integer topicId) {
    ...
    try {
      Integer count =
          agencyServiceJdbcTemplate.queryForObject(sql, Integer.class, agencyId, topicId);
